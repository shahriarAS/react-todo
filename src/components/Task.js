    ...
    return (
        <li className="border-b-2 border-green-700 flex justify-between">
            <div className="cursor-pointer">
                <i onClick={markComplete} className="fa fa-check-square mr-2"></i>
                {edit ? (
                    <form className="w-full max-w-sm" onSubmit={updateTask}>
                        <div className="flex items-center border-b border-teal-500 py-2">
                            <input className="appearance-none bg-transparent border-none w-full text-gray-700 mr-3 py-1 px-2 leading-tight focus:outline-none" type="text" placeholder="Jane Doe" aria-label="Full name" value={edit} onChange={event => setEdit(event.target.value)} />
